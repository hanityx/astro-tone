...
Its style lives in `src/styles/components/figure-note.css` and uses the same tokens as the rest of the theme. If you change `--color-blue` or `--label-tertiary`, the note follows.
...
Component CSS lives in `src/styles/components/`. Import it from the top of the `.astro` file:

```astro
---
import '../styles/components/figure-note.css';
---
```

Astro hoists the import and includes the CSS once, no matter how many posts use the component.
